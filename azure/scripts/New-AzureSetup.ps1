...
Set-AzKeyVaultSecret `
   ...
   -SecretValue $spokeSaBlobContainerName
...
if (Get-AzBillingAccount)
{
    ...
    $secureBillingAccountName = ConvertTo-SecureString -String $billingAccountName -AsPlainText -Force
    Set-AzKeyVaultSecret `
       -VaultName $KeyvaultName `
       -Name "BillingAccountName" `
       -SecretValue $secureBillingAccountName

    $secureBillingProfileName = ConvertTo-SecureString -String $billingProfileName -AsPlainText -Force
    Set-AzKeyVaultSecret `
       -VaultName $KeyvaultName `
       -Name "BillingProfileName" `
       -SecretValue $secureBillingProfileName

    # Ensure that $InvoiceSection is a string.
    # If $InvoiceSection.name is the right value, then use that.
    # I'm assuming that $InvoiceSection.name gives the desired string value.
    $secureInvoiceSectionName = ConvertTo-SecureString -String $invoiceSectionName -AsPlainText -Force
    Set-AzKeyVaultSecret `
       -VaultName $KeyvaultName `
       -Name "InvoiceSectionName" `
       -SecretValue $secureInvoiceSectionName

}
else
{
    Write-Host "Can't get billing info"
}
